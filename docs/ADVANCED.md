...
- **Manual transforms belong in log, not linear.** Any non-AI pixel geometry (scale, rotate, distort, warp, skew, any resample) crushes highlight / shadow detail when done on linear data. Wrap it: convert Linear->Log, do the transform, convert Log->Linear. We already ship the node for this, `REDACTEDLogConvert` (ACEScct, HDR-safe, reversible; `operation` = linear_to_log / log_to_linear); Nuke's equivalent is `OCIOLogConvert`. A production practice; carry float through the wrap and persist EXR via `SaveImageAdvanced` / `REDACTEDSave` if needed. Full entry + the native EXR/linear path: `docs/NODE_LIBRARY/color-and-transform.md`.
